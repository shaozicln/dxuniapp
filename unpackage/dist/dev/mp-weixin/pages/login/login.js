...
const _sfc_main = {
  data() {
    common_vendor.index.__f__("log", "at pages/login/login.vue:48", "登录页组件初始化 - data数据初始化完成");
    return {
      showLoginForm: false,
      // 是否显示登录表单
      form: {
  ...
  onLoad() {
    common_vendor.index.__f__("log", "at pages/login/login.vue:63", "登录页onLoad生命周期触发 - 开始执行初始化逻辑");
    this.initLoginFlow();
  },
  methods: {
    showMsg(message, type = "error") {
      common_vendor.index.__f__("log", "at pages/login/login.vue:70", `[消息提示] 类型: ${type}, 内容: ${message}`);
      if (this.msgTimer) {
      ...
      this.msgTimer = setTimeout(() => {
        common_vendor.index.__f__("log", "at pages/login/login.vue:77", "消息提示自动关闭");
        this.msg = "";
      }, 5e3);
    },
    /**
     ...
     */
    async initLoginFlow() {
      common_vendor.index.__f__("log", "at pages/login/login.vue:87", "进入initLoginFlow - 启动自动登录流程（获取code+请求token）");
      this.isLoading = true;
      try {
        const wxLoginRes = await new Promise((resolve, reject) => {
          common_vendor.index.login({
            ...
            success: (res) => {
              if (res.code) {
                common_vendor.index.__f__("log", "at pages/login/login.vue:96", "自动获取微信code成功:", res.code);
                resolve(res.code);
              } else {
        ...
        common_vendor.index.__f__("log", "at pages/login/login.vue:109", "自动调用后端/loginByCode接口，传递code请求token");
        const autoLoginRes = await src_utils_request_request.post("https://jxpj.neau.edu.cn/api/v1/loginByCode", {
          weixincode: wxLoginRes
          // 传递自动获取的微信code
        });
        if (autoLoginRes == null ? void 0 : autoLoginRes.token) {
          common_vendor.index.__f__("log", "at pages/login/login.vue:116", "自动登录成功，获取到token:", autoLoginRes.token);
          common_vendor.index.setStorageSync("autoToken", autoLoginRes.token);
          this.showMsg("自动获取登录凭证成功", "success");
        } else {
          common_vendor.index.__f__("log", "at pages/login/login.vue:121", "自动登录未获取到有效token，需后续用户手动登录");
          this.showMsg("未获取到相关信息，请手动登录", "error");
        }
      } catch (err) {
        common_vendor.index.__f__("error", "at pages/login/login.vue:126", "自动登录流程异常:", err);
        this.showMsg("自动登录失败，请重试", "error");
      } finally {
        await this.checkLocalToken();
        this.isLoading = false;
        common_vendor.index.__f__("log", "at pages/login/login.vue:132", "自动登录流程完成，进入本地Token校验阶段");
      }
    },
    // 校验本地Token：有则直接跳转，无则显示登录表单（逻辑保留，增加兼容性）
    async checkLocalToken() {
      common_vendor.index.__f__("log", "at pages/login/login.vue:138", "进入checkLocalToken方法 - 开始校验本地登录状态");
      this.isLoading = true;
      try {
        ...
        common_vendor.index.__f__("log", "at pages/login/login.vue:146", "从本地存储获取数据:", {
        ...
        if ((localAutoToken || localManualToken) && (localUser == null ? void 0 : localUser.teacherId)) {
          common_vendor.index.__f__("log", "at pages/login/login.vue:154", "本地验证通过: 存在有效Token和完整用户信息");
          ...
          common_vendor.index.__f__("log", "at pages/login/login.vue:162", "本地无有效Token，需要用户手动登录");
          this.showLoginForm = true;
        }
      } catch (err) {
        common_vendor.index.__f__("error", "at pages/login/login.vue:167", "Token校验过程发生异常:", err);
        this.showMsg("登录状态校验异常，请重试");
        this.showLoginForm = true;
      } finally {
        this.isLoading = false;
        common_vendor.index.__f__("log", "at pages/login/login.vue:172", "checkLocalToken方法执行完成");
      }
    },
    /**
     ...
     */
    async handleLogin() {
      common_vendor.index.__f__("log", "at pages/login/login.vue:180", "进入handleLogin方法 - 执行用户手动登录逻辑");
      this.isLoading = true;
      try {
        ...
        const loginRes = await src_utils_request_request.post("https://jxpj.neau.edu.cn/api/v1/login", {
          weixincode: wxLoginRes,
          username: this.form.teacherId.trim(),
          password: this.form.teacherId.trim()
        });
        if (loginRes == null ? void 0 : loginRes.token) {
          common_vendor.index.__f__("log", "at pages/login/login.vue:206", "手动登录成功，获取到token:", loginRes.token);
          const userInfo = {
        ...
        common_vendor.index.__f__("error", "at pages/login/login.vue:231", "手动登录异常:", err);
  ...
  onUnload() {
    common_vendor.index.__f__("log", "at pages/login/login.vue:241", "登录页onUnload生命周期触发");
    if (this.msgTimer) {
      clearTimeout(this.msgTimer);
    }
  }
